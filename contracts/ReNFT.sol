...
contract ReNFT is IReNFT, ERC721Holder, ERC1155Receiver, ERC1155Holder {
    ...
    function handleLend(IReNFT.CallData memory cd) private {
        for (uint256 i = cd.left; i < cd.right; i++) {
            ...
            IReNFT.Lending storage lending = lendings[identifier];
            ensureIsNull(lending);
            ensureTokenNotSentinel(uint8(cd.paymentToken[i]));
            bool is721 = cd.nftStandard[i] == IReNFT.NFTStandard.E721;
            lendings[identifier] = IReNFT.Lending({
    ...
    function handleStopLend(IReNFT.CallData memory cd) private {
        ...
        for (uint256 i = cd.left; i < cd.right; i++) {
            ...
            emit IReNFT.StopLend(cd.lendingID[i], uint32(block.timestamp));
    ...
    function handleRent(IReNFT.CallData memory cd) private {
        for (uint256 i = cd.left; i < cd.right; i++) {
            bytes32 lendingIdentifier = keccak256(
                abi.encodePacked(
                    cd.nftAddress[cd.left],
            ...
            IReNFT.Lending storage lending = lendings[lendingIdentifier];
            IReNFT.Renting storage renting = rentings[rentingIdentifier];
            ensureIsNotNull(lending);
            ...
            rentings[rentingIdentifier] = IReNFT.Renting({
    ...
    function handleStopRent(IReNFT.CallData memory cd) private {
        for (uint256 i = cd.left; i < cd.right; i++) {
            bytes32 lendingIdentifier = keccak256(
                abi.encodePacked(
                    cd.nftAddress[cd.left],
            ...
            emit IReNFT.StopRent(cd.rentingID[i], uint32(block.timestamp));
    ...
    function bundleCall(
        ...
        while (cd.right != cd.nftAddress.length) {
            if (
                ...
                (cd.nftStandard[cd.right] == IReNFT.NFTStandard.E1155)
            ) {
                cd.right++;
            } else {
                handler(cd);
    ...
    function distributePayments(
        IReNFT.Lending memory lending,
        IReNFT.Renting memory renting,
    ...
    function safeTransfer(
        ...
        if (cd.nftStandard[cd.left] == IReNFT.NFTStandard.E721) {
        ...
        )
    {
        ...
        IReNFT.Renting storage renting = rentings[identifier];
        return (
